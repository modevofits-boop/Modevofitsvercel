...
const seedSettings = () => ({ storeName:"Modevofits", currency:"PKR", freeShippingThreshold:4000, shippingFee:200, expressFee:350, announcement:"Complimentary delivery over ₨ 4,000 — nationwide", whatsapp:"" });
...
async function createProduct(data) {
  ...
  const prod = {
    ...
    sizeChart: data.sizeChart && Array.isArray(data.sizeChart.rows) && data.sizeChart.rows.length ? data.sizeChart : null,
...
async function updateProduct(id, data) {
  ...
  if (!p) return null;
  ["name","category","type","colors","sizes","desc","image","images","featured","active"].forEach((f) => { if (f in data) p[f] = data[f]; });
  if ("description" in data) p.desc = data.description;
  if ("sizeChart" in data) p.sizeChart = data.sizeChart && Array.isArray(data.sizeChart.rows) && data.sizeChart.rows.length ? data.sizeChart : null;
...
async function createOrder({ items, customer, shipping, payment }) {
  ...
  for (const it of items) {
    ...
    clean.push({ productId: prod.id, name: prod.name, type: prod.type, category: prod.category, color: it.color, hex: it.hex, size: it.size, price: prod.price, cost: prod.cost || 0, qty });
