...
module.exports = async (req, res) => {
  try {
    ...
    if (action === "stats") {
      ...
      // sales by category
      const catMap = {};
      orders.forEach((o) => o.items.forEach((i) => { const c = i.category || (products.find((p)=>p.id===i.productId)||{}).category || "Other"; if (!catMap[c]) catMap[c] = { revenue: 0, units: 0 }; catMap[c].revenue += i.price * i.qty; catMap[c].units += i.qty; }));
      const byCategory = Object.entries(catMap).map(([category,v])=>({category,revenue:v.revenue,units:v.units})).sort((a,b)=>b.revenue-a.revenue);
    ...
    if (action === "export") {
      ...
      const head = ["Order","Date","Status","Payment","Customer","Phone","Email","Address","City","Items","Subtotal","Delivery","Total"];
      const lines = [head.map(esc).join(",")];
      orders.forEach((o) => {
        const items = o.items.map((i) => `${i.name} (${i.color||""}/${i.size||""}) x${i.qty}`).join(" | ");
        lines.push([o.number, new Date(o.createdAt).toISOString().slice(0,10), o.status, o.payment, o.customer.name, o.customer.phone, o.customer.email||"", o.customer.address, o.customer.city, items, o.subtotal, o.shipping, o.total].map(esc).join(","));
      });
      res.setHeader("Content-Type", "text/csv; charset=utf-8");
      res.setHeader("Content-Disposition", 'attachment; filename="modevofits-orders.csv"');
